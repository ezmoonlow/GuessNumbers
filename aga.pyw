    ...
    for e in pygame.event.get():
        if e.type == pygame.QUIT:
            ...
            if e.key == pygame.K_ESCAPE:
                ...
                if int(numeral) > 100:
                    dialogs('', OUTSIDE_BG, 'Я же говорю , число 0-100')
                ...
                if move == 1:
                    if int(numeral) == number:
                        ...
                        dialogs('Собака, твой ход', dialog_cat_pos, 'Продолжаем')

                if move == 2:
                    if int(numeral) == number:
                        dialogs(f'Это число {numeral}', dialog_dog_pos, 'Собака, ты выйграла')
                        block = 1
                    else:
